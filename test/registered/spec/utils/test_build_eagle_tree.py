...
class TestBuildEagleTree(unittest.TestCase):
    ...
    def test_filter_batch_handles_compacted_state_with_old_request_indices(self):
        draft_input = EagleDraftInput(
            hidden_states=torch.tensor(
                [
                    [1.0, 2.0],
        ...
        draft_input.filter_batch(
            torch.tensor([0, 4, 7], device=get_device(), dtype=torch.int64),
            has_been_filtered=True,
        )
        ...
        self.assertTrue(
            torch.equal(
                draft_input.verified_id.cpu(),
                torch.tensor([101, 102, 103, 104, 105, 106], dtype=torch.int32),
            )
        )
        self.assertEqual(draft_input.accept_length_cpu, [0, 2, 1])
        self.assertTrue(
            torch.equal(
                draft_input.req_pool_indices_for_draft_extend.cpu(),
